...
// did not have. Pick a backend by setting EXPO_PUBLIC_BACKEND_PROVIDER;
// env-check then enforces that provider's required vars.
...
export function resolveRuntimeConfig(env = {}) {
    const backendProvider = normalizeEnumValue(env.EXPO_PUBLIC_BACKEND_PROVIDER)
    const authMode = normalizeEnumValue(env.EXPO_PUBLIC_AUTH_MODE)
