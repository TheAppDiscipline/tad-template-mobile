...
function runTsxWithEnv(script, env) {
...
test('FIREBASE mobile auth uses email/password and rejects magic links', async () => {
  ...
  const backend = createFirebaseBackend({
    ...
    db: {},
    firestore: minimalFirestore(),
    createUserWithEmailAndPassword: async (_auth, email, password) => calls.push(['signUp', email, password]),
    signInWithEmailAndPassword: async (_auth, email, password) => calls.push(['signIn', email, password]),
  ...
  await backend.auth.signInEmailPassword('owner@example.com', 'secret-123')
  await backend.auth.signUpEmailPassword('new@example.com', 'secret-456')
  await assert.rejects(
    () => backend.auth.signInMagicLink('owner@example.com'),
    /Firebase Mobile magic link requires a verified HTTPS callback; use EMAIL_PASSWORD or Supabase for magic link/,
  )

  assert.deepEqual(calls, [
    ['signIn', 'owner@example.com', 'secret-123'],
    ['signUp', 'new@example.com', 'secret-456'],
  ])
})

test('LOCAL_ONLY mobile auth implements optional email/password contract deterministically', async () => {
  ...
  const backend = createLocalBackend({
    ...
    randomUUID: () => 'local-user-1',
    now: () => '2026-06-10T00:00:00.000Z',
  })

  await backend.auth.signUpEmailPassword('local@example.com', 'ignored')
  assert.deepEqual(await backend.auth.getUser(), {
    id: 'local-user-1',
...
// terminal plus green for reentry). Otherwise it abstains (null), so watcher and skill share the SAME advance
...
test('watch detectNext authorizes a Step 4 advance only when the origin is coherent', () => {
  ...
  const COMPLETION_PARTIAL = ['## SLICE_COMPLETION_PACKET', '', '### Slice', '- Slice 1', '', '### Outcome', '- partial', '', '### Gates', '- GATE_STATE: passed', ''].join('\n')
  ...
  // reentry advances only when the outcome closes the slice and the completion gate is green.
  assert.equal(detect({ 'STEP_4_EXECUTION_PACKET.md': EXEC_VALIDATED, 'SLICE_COMPLETION_PACKET.md': COMPLETION_PASSED }), '4-reentry')
  assert.equal(detect({ 'STEP_4_EXECUTION_PACKET.md': EXEC_VALIDATED, 'SLICE_COMPLETION_PACKET.md': COMPLETION_PARTIAL }), null)
...
test('discipline:step4-origin resolves the origin fail-loud', () => {
  ...
  const COMPLETION_PARTIAL = ['## SLICE_COMPLETION_PACKET', '', 'STATUS: ready', '', '### Slice', '- Slice 1', '', '### Outcome', '- partial', '', '### Gates', '- GATE_STATE: passed', ''].join('\n')
  ...
  // reentry with a green gate but a partial outcome stays open -> invalid
  r = resolve({ 'STEP_4_EXECUTION_PACKET.md': EXEC_VALIDATED, 'SLICE_COMPLETION_PACKET.md': COMPLETION_PARTIAL })
  assert.equal(r.exit, 2, r.raw)
  assert.equal(r.json.status, 'invalid')
  assert.match(r.json.reason, /partial.*leaves the slice open/i)
...
test('checkpoint: approve records --reason in the Decision and the ledger', () => {
  const gitProbe = spawnSync('git', ['--version'], { encoding: 'utf8' })
  if (gitProbe.status !== 0) return
  ...
  assert.match(ledger, /"event":"checkpoint_decided"[^\n]*"reason":"risk accepted by owner"/)
  ...
  assert.equal(plain.status, 0, getOutput(plain))
  ...
  assert.equal(plainApprove.status, 0, getOutput(plainApprove))
  assert.doesNotMatch(fs.readFileSync(path.join(packetsDir, plainFile), 'utf8'), /Reason:/)
  ...
  assert.match(getOutput(usage), /approve <packet-file-or-id> \[--reason/)
  ...
  assert.match(getOutput(wrapperHelp), /approve <packet-file-or-id> \[--reason/)
})
...
function makeRunFixtureRepo(overrides = {}) {
  ...
  fs.writeFileSync(
    ...
    ['# discipline.md', '', '## 0) Profile', '- PROFILE: LITE', '- LANE: MOBILE', '', '## Autonomy', `- level: ${level}`, '- builder: claude', '- validator: gemini', '- repair_max: 2', '', '## 1) Non-Negotiables', '- x', ''].join('\n'),
    'utf8',
  )
  fs.writeFileSync(
...
test('gate --changed: the buyer paths route to the gates this lane actually needs', () => {
  ...
  assert.ok(
    out.api.scripts.includes('check-mobile-release'),
    `an API change must run this lane's artifact check (check-mobile-release): ${out.api.scripts.join(', ')}`,
  )
...
const AUTHENTICATED_TEST = '.maestro/authenticated/signed-in.yaml'
const MISPLACED_TEST = '.maestro/signed-in.yaml'
...
// The Mobile lane had no artifact check at all: `deployment-artifact` ran lint, types, tests,
// secrets and HTTPS, none of which look at app.json. A build could therefore ship under the
// template's own bundle id, with a missing icon, and every gate stayed green.
test('check-mobile-release: the artifact is checked, not assumed', () => {
  const project = (app, eas = { build: { production: {} } }) => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mobile-artifact-'))
    fs.writeFileSync(path.join(dir, 'app.json'), JSON.stringify({ expo: app }, null, 2), 'utf8')
  ...
  const template = run(project({ ...REAL, ios: { bundleIdentifier: 'com.theappdiscipline.app' } }))
  assert.notEqual(template.status, 0, getOutput(template))
  assert.match(getOutput(template), /still the template's/)

  // A missing identifier cannot be signed or uploaded at all.
  const noPackage = run(project({ ...REAL, android: {} }))
  assert.notEqual(noPackage.status, 0, getOutput(noPackage))
  assert.match(getOutput(noPackage), /expo\.android\.package is missing/)

  // An asset path that does not resolve produces a build with a blank icon.
  const badIcon = run(project({ ...REAL, icon: './assets/does-not-exist.png' }))
  assert.notEqual(badIcon.status, 0, getOutput(badIcon))
  assert.match(getOutput(badIcon), /does not exist/)

  // No build profile means `eas build` has nothing to run.
  const noProfiles = run(project(REAL, { build: {} }))
  assert.notEqual(noProfiles.status, 0, getOutput(noProfiles))
  assert.match(getOutput(noProfiles), /no build profiles/)
