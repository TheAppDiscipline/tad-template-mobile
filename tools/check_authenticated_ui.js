 ...
 *   2. No flow under `.maestro/authenticated/`. `npm run e2e:auth` runs that
 *      directory on a device after this check passes, so an empty one means the
 *      surface routes to nothing.
 *
 ...
 * does the rest.
 *
 * Exit 0 = an authenticated suite exists and e2e:auth will run it on a device.
