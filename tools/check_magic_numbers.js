 ...
 * tokens (NN #9).
 *
 * Mobile adaptation vs. web:
 *   - Also scans App.tsx at repo root (Expo entry point).
 *   - Allowed tokens files include src/theme/tokens.{ts,css} in addition to
 *     src/styles/tokens.{ts,css} (React Native convention varies).
 *
 * This check is a SEMI warning (fuzzy heuristic). Exits 0 on violations
 * unless `--strict` is passed (then exits 1).
 *
...
const ROOT_APP_FILE = path.join(ROOT, 'App.tsx');
const STRICT = process.argv.includes('--strict');

const ALLOWED_FILES = new Set([
  path.normalize(path.join(SRC_DIR, 'styles', 'tokens.css')),
  path.normalize(path.join(SRC_DIR, 'styles', 'tokens.ts')),
  path.normalize(path.join(SRC_DIR, 'theme', 'tokens.css')),
  path.normalize(path.join(SRC_DIR, 'theme', 'tokens.ts')),
]);

const SCAN_EXT = new Set(['.tsx', '.jsx', '.ts']);
const IGNORE_DIRS = new Set(['node_modules', '.git', 'dist', '.tmp', 'coverage', '.expo']);

// Props where inline numeric values usually indicate magic numbers.
// React Native uses unit-less numbers (dp), so px/rem suffixes also covered for shared libs.
const STYLE_PROPS = [
  'fontSize',
  'fontWeight',
  'lineHeight',
...
console.log('--- Visual Gate: No Magic Numbers in UI (mobile) ---');

const files = [];
if (fs.existsSync(SRC_DIR)) files.push(...walk(SRC_DIR));
if (fs.existsSync(ROOT_APP_FILE)) files.push(ROOT_APP_FILE);

if (files.length === 0) {
  console.log('\x1b[33m[SKIP]\x1b[0m No src/ directory and no App.tsx at root.');
  process.exit(0);
}
...
console.log('Fix: replace literal values with tokens from src/theme/tokens.{ts,css} or src/styles/tokens.{ts,css}.');
