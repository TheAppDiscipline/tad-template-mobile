...
import { isTerminalOutcome, readOutcome } from './completion-packet.js';
 ...
 * validated for EVERY mode, terminal outcome plus green completion gate for reentry, feedback branch declared for
...
function modeCoherenceIssues(root: string, mode: Step4Mode): string[] {
  ...
  if (mode === '4-reentry') {
    const completion = readPacket(root, 'SLICE_COMPLETION_PACKET.md');
    if (completion === null) {
      issues.push('SLICE_COMPLETION_PACKET is missing for a reentry.');
    } else {
      const outcome = readOutcome(completion);
      if (!outcome.ok) {
        issues.push(`the slice completion outcome is contradictory (${outcome.reason}); resolve it before reentry.`);
      } else if (!isTerminalOutcome(outcome.outcome)) {
        issues.push(`the slice completion outcome is "${outcome.outcome ?? 'unstated'}", which leaves the slice open; finish Step 5 before reentry.`);
      }
...
function chosenReason(mode: Step4Mode): string {
  switch (mode) {
    ...
    case '4-reentry': return 'A SLICE_COMPLETION_PACKET closes the slice with a terminal outcome and its completion gate passed.';
