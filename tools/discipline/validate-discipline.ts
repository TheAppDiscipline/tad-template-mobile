...
export function validateDiscipline(root: string): ValidationIssue[] {
  ...
  if (fs.existsSync(pendingDir)) {
    ...
    if (pending.length > 0) issues.push({ severity: 'error', message: `${pending.length} pending patch(es)`, detail: 'For manual mode, stop discipline:watch with Ctrl+C before saving new packets or patches. Automatic mode computes the hash and applies all pending patches without individual approval. Run npm run discipline:patch:dry-run and review all pending files: every row must show [OK]. Do not continue on [FAIL]; exit code zero alone does not confirm applicability. After human approval, replace HASH_REVISADO with the 64-character BATCH_SHA256 in npm run discipline:patch -- --approve-sha HASH_REVISADO. If pending files change, repeat the preview and review. In PowerShell use npm.cmd. Then inspect git diff and run npm run discipline:validate; resolve failures before the next handoff.' });
  }
