...
- `.discipline/patches/pending/` is empty (if there are patches, apply them first with `npm run discipline:patch -- --approve-sha HASH_REVISADO`).
...
2. Look for a direct fix in the common gate errors note in the vault (sold separately).
...
Write `.discipline/packets/SLICE_COMPLETION_PACKET.md` using the canonical structure from the vault (Step 5 - Implementation, the "minimal SLICE_COMPLETION_PACKET structure" section, in The App Discipline vault, sold separately):
...
If `Deploy signal != not_ready`, also generate `.discipline/packets/DEPLOY_READINESS_PACKET.md` using the vault structure (Step 5 - Implementation, the "minimal DEPLOY_READINESS_PACKET structure" section, in the vault, sold separately).
...
- If patches are pending and `discipline:patch` fails: stop, report the conflict, redirect to the apply-patch-blocks note in the vault (sold separately).
