...
2. Buscar fix directo en la nota "Errores Comunes del Gate" del vault.
...
Escribir `.discipline/packets/SLICE_COMPLETION_PACKET.md` con la estructura canonica del vault (Paso 5 - Implementación, sección "Estructura mínima de SLICE_COMPLETION_PACKET"):
...
Si `Deploy signal != not_ready`, generar tambien `.discipline/packets/DEPLOY_READINESS_PACKET.md` con la estructura del vault (Paso 5 - Implementación, sección "Estructura mínima de DEPLOY_READINESS_PACKET").
...
- Si patches pendientes y `discipline:patch` falla: parar, reportar conflicto, redirigir a la nota "Aplicar Patch Blocks" del vault.
