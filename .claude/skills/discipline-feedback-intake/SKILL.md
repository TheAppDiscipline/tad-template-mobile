...
`npm run discipline:patch -- --approve-sha HASH_REVISADO` applies EVERYTHING in pending/, so running intake now would apply that
...
- If `npm run discipline:patch -- --approve-sha HASH_REVISADO` fails: report the exact error, leave the patch files in `.discipline/patches/pending/` (they are the operator's evidence), do not retry blindly. The next run's preflight will surface them.
