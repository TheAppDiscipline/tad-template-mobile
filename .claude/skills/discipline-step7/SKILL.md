...
- If `npm run discipline:patch -- --approve-sha HASH_REVISADO` fails: stop. Report the failing patch and the engine's error verbatim. The batch was rolled back and every block is back in `.discipline/patches/pending/`; do not assemble, do not log, do not summarize. Never present a PROFILE change as applied when its patch failed.
