...
NOTA: el skill no decide el lane por el usuario. Si dudas qué lane usar, primero corre `/discipline-step0a` (que produce IDEA_VALIDATION_PACKET) y/o consulta la guía "Elegir Lane" del vault. Después invoca este skill con el lane confirmado.
...
5. Verifica versión Node ≥ 20, Git ≥ 2.40 (según la guía "Setup para No Programadores" del vault).
...
Si eres no-programador, ver instrucciones paso a paso en la guía "Setup para No Programadores" del vault.
```
...
- Si persiste, mostrar error y referir al usuario a la nota "Errores Comunes del Gate" (sección Windows / entorno local) del vault o el equivalente del SO.
...
Catalogo completo de skills: en el vault (Biblioteca de Skills Discipline Loop).
