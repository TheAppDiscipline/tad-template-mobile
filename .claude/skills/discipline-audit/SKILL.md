...
This skill runs one or all 15 of the auto-audit prompts (documented in The App Discipline vault, sold separately) in a single session. It turns 15 manual copy/paste operations into one command, captures structured output, and logs each run to `findings.md §Audits`.
...
- Read the canonical list of 24 NN from `.claude/skills/discipline-step0a/SKILL.md` or the "Discipline Loop" note in the vault (sold separately).
