...
`npm run discipline:patch -- --approve-sha HASH_REVISADO` applies EVERYTHING in pending/, so running this step now would apply that
unrelated work together with this step's own blocks, in the same silent operation. Resolve them
first, then re-run this step:

- read them, and if they still hold, apply them consciously with `npm run discipline:patch -- --approve-sha HASH_REVISADO`; or
...
```bash
npm run discipline:patch -- --approve-sha HASH_REVISADO
```
...
- If `npm run discipline:patch -- --approve-sha HASH_REVISADO` fails: stop. Report the failing patch and the engine's error verbatim. The batch was rolled back and every block is back in `pending/`; do not assemble, do not log, do not summarize. Fixing the block and re-running the step is the operator's call.
...
- If the validated architecture changes `BACKEND_PROVIDER` or `AUTH_MODE` (via `DISCIPLINE_MD_PATCH_BLOCK`), tell the operator to run `npm run discipline:provider:generate` right after `npm run discipline:patch -- --approve-sha HASH_REVISADO` and to commit the regenerated `src/config/provider.generated.*`; `npm run gate` fails on a stale provider artifact.
