...
- If `npm run discipline:patch -- --approve-sha HASH_REVISADO` fails: stop. Report the failing patch and the engine's error verbatim. The batch was rolled back and every block is back in `pending/`; do not assemble and do not present the step as complete. The deploy stands and its packet stays in `.discipline/packets/`: log the run as INCOMPLETE so the deploy is on record without claiming the handoff exists.
