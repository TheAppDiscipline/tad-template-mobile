...
- `validate-scorecard.ts` does not exist: the template is not post-Wave 3.1. Suggest updating to the current version or installing the script manually per the implementation-status note in the vault (sold separately).
