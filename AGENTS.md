...
Choose manual mode when you want to review and approve each batch. For manual mode, stop `discipline:watch` with `Ctrl+C` before saving new packets or patches. Automatic mode (`discipline:watch`) computes the batch hash and applies all pending patches without asking for individual approval. Use automatic mode only when you want saving a packet with patch blocks to process all pending patches immediately.

Run `npm run discipline:patch:dry-run` and review every file in `.discipline/patches/pending/`. All rows must show `[OK]`. Do not continue if `[FAIL]` appears; a zero exit code alone does not confirm that the patches can be applied. The reason may appear inside the marker, for example `[FAIL (anchor not found)]`. Replace `HASH_REVISADO` with the 64 characters printed after `BATCH_SHA256` only after the human approves that exact batch. Do not auto-approve the printed hash. If pending files change, repeat the preview and review. In Windows PowerShell use `npm.cmd` instead of `npm`. After applying, inspect `git diff` and run `npm run discipline:validate`; resolve failures before assembling the next handoff.
...
Start `npm run discipline:watch` only when the operator chooses automatic batch application as described above.

That watcher is responsible for:
- reading new packets in `.discipline/packets/`
- extracting patch blocks
- computing the batch hash and applying all pending patches without individual approval
- updating `progress.md` after `SLICE_COMPLETION_PACKET`
- assembling the next `paste-ready` handoff

For review and approval of each batch, use manual mode with the watcher stopped before saving new packets or patches.
...
```bash
npm run gate        # Expo/fork compatibility + asset preflight + lint + typecheck + tests + local security gates
npm run gate:full   # gate + magic-number/query scans + AI fixture eval
npm run check-expo-compat   # official Expo dependency alignment; needs registry access
npm run check-vendored-deps # rebuild and verify the integrity-pinned image-size fork tarball
...
```bash
npm run discipline:patch -- --approve-sha HASH_REVISADO      # apply pending patches to discipline.md/task_plan.md/findings.md/progress.md
...
|---|---|---|
| `ui` | `gate:visual` | A screen that renders wrong passes every static check. Needs the configured visual runner or the Maestro/device tooling (`npm run e2e:smoke`). |
...
For UI lanes, you can also run the minimal Maestro smoke on a device/emulator:

```bash
npm run e2e:smoke   # maestro test .maestro/smoke.yaml
npm run e2e         # maestro test .maestro/
```

That check is intentionally separate from `npm run gate` so the project can adopt device verification progressively. It requires the Maestro CLI installed and a running emulator/simulator.
