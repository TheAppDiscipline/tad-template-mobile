...
Choose manual mode when you want to review and approve each batch. For manual mode, stop `discipline:watch` with `Ctrl+C` before saving new packets or patches. Automatic mode (`discipline:watch`) computes the batch hash and applies all pending patches without asking for individual approval. Use automatic mode only when you want saving a packet with patch blocks to process all pending patches immediately. For the manual procedure, see [Pipeline Automation](#pipeline-automation-discipline-scripts).

Start the watcher only if you choose automatic batch application:
...
For approval of each batch, use manual mode with the watcher stopped before saving new packets or patches.
...
For manual application, stop `discipline:watch` with `Ctrl+C` before saving new packets or patches. Run `npm run discipline:patch:dry-run` and review every file in `.discipline/patches/pending/`. All rows must show `[OK]`. Do not continue if `[FAIL]` appears; a zero exit code alone does not confirm that the patches can be applied. The reason may appear inside the marker, for example `[FAIL (anchor not found)]`. Replace `HASH_REVISADO` with the 64 characters printed after `BATCH_SHA256` only when you approve that exact batch. If pending files change, repeat the preview and review. In Windows PowerShell use `npm.cmd` instead of `npm`. After applying, inspect `git diff` and run `npm run discipline:validate`; resolve failures before assembling the next handoff.
...
npm run discipline:patch -- --approve-sha HASH_REVISADO      # Apply pending patch blocks to discipline.md/task_plan.md/findings.md/progress.md
